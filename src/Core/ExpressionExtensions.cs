namespace Core
{
    ...
    public static class ExpressionExtensions
    {
        ...
        {
            return expression1.Compose<Func<T, Boolean>>(expression2, Expression.OrElse);
        }
        ...
        {
            return expression1.Compose<Func<T, Boolean>>(expression2, Expression.AndAlso);
        }

        public static Expression<Func<T, Boolean>> Not<T>(this Expression<Func<T, Boolean>> expression1)
        {
            return Expression.Lambda<Func<T, Boolean>>(Expression.Not(expression1.Body), expression1.Parameters);
        }
